...
This specification defines the under-the-hood combat runtime used by the mockups and eventually by the battle system.

The runtime is intentionally headless. Rendering, animation, browser timing, and battle presentation consume simulation output but do not control authoritative combat state.

## Hexframe Timing Contract

The runtime follows the same core deterministic timing rules used by the Hexframe combat engine:

- **60 fixed combat ticks per second**
- one call to `step()` advances exactly one tick
- combat runtime code never reads browser `deltaTime`, `Date.now()`, or animation-frame duration
- authoritative arithmetic is integer-only
- percentages use **10,000 basis points**
- seeded RNG lives inside authoritative combat state
...
No floating-point percentage is required by the authoritative combat runtime.
...
The runtime accepts selected talent names and compiles them into deterministic hook descriptors.
...
- same authoritative combat runtime loop as larger content
...
Every combat tick produces a frame report.
...
Any mismatch is a combat-runtime bug.
