...
const STATUS_GROUPS = Object.freeze([
  Object.freeze({
    status: "available",
    listId: "availableQuestList",
    countId: "availableQuestCount"
  }),
  Object.freeze({ status: "active", listId: "activeQuestList", countId: "activeQuestCount" }),
  Object.freeze({
    status: "completed",
    listId: "completedQuestList",
    countId: "completedQuestCount"
  })
]);
...
function rewardText(quest) {
  return (
    fmt(quest.reward?.gold) +
    " gold · " +
    fmt(quest.reward?.amount) +
    " quest mark" +
    (Number(quest.reward?.amount) === 1 ? "" : "s")
  );
}

function assignedHeroNames(quest) {
  return (quest.heroIds || [])
    .map(id => Roster.hero(id))
    .filter(Boolean)
    .map(hero => hero.name);
}
...
function emptyState(status) {
  ...
  return (
    '<div class="quest-journal__empty"><span class="wow-kicker">EMPTY</span><p>' +
    copy +
    "</p></div>"
  );
}
...
function questRow(quest) {
  ...
  const heroes = assignedHeroNames(quest);
  const status =
    quest.status === "active" ? "active" : quest.status === "completed" ? "completed" : "available";
  ...
  const statusCopy =
    status === "active"
      ? heroes.length
        ? heroes.join(", ")
        : quest.requiredHeroes + " heroes dispatched"
      : status === "completed"
        ? "Completed " +
          (quest.completedCount || 1) +
          " time" +
          ((quest.completedCount || 1) === 1 ? "" : "s")
  ...
  article.innerHTML =
    ...
    Icons.resolve("currency", "gold") +
    '" alt=""></span>' +
    "<span><strong>" +
    fmt(quest.reward?.gold) +
    "</strong><small>Gold</small></span>" +
    "</span>" +
    '<span class="quest-journal__marks">' +
    '<span class="wow-icon-frame wow-icon-frame--xs"><img src="' +
    Icons.resolve("quest", "marks") +
    '" alt=""></span>' +
    "<span><strong>" +
    fmt(quest.reward?.amount) +
    "</strong><small>Marks</small></span>" +
    "</span>";
...
function renderGroup(group, quests) {
  ...
  matching
    .sort((a, b) => (b.round || 0) - (a.round || 0) || (a.difficulty || 1) - (b.difficulty || 1))
    .forEach(quest => root.appendChild(questRow(quest)));
}
...
function renderJournal() {
  ...
  const faction =
    typeof Roster.getFaction === "function" ? Roster.getFaction() : snapshot.faction || "alliance";
...
async function renderAutomation() {
  const root = $("questAutomation"),
    board = $("questAutomationBoard");
  if (Campaign.getBaseLevel() < 2) {
    root.hidden = true;
    board.innerHTML = "";
    return;
  }
  root.hidden = false;
  if (!Progression.getCatalog()) {
    const data = await fetch("../data/content/progression.json").then(r => r.json());
    Progression.configure(data);
    ContentAssignments.configure(data);
  }
  const offers = Roster.getState().quests.filter(q => q.status === "available");
  const state = ContentAssignments.state("quest");
  board.innerHTML = "";
  state.slots.forEach((assignment, index) => {
    const slot = document.createElement("article");
    slot.className = "assignment-slot " + (assignment ? "is-filled" : "is-empty");
    slot.dataset.assignmentSlot = String(index);
    const hero = assignment && Roster.hero(assignment.heroId);
    slot.innerHTML =
      '<div class="assignment-slot__label"><span>AUTO QUEST ' +
      (index + 1) +
      "</span><small>" +
      (assignment ? "Assigned" : "Drop hero") +
      '</small></div><div class="assignment-slot__body">' +
      (hero
        ? "<strong>" + hero.name + "</strong><small>" + assignment.contentId + "</small>"
    ...
    slot.addEventListener("drop", e => {
      e.preventDefault();
      const heroId =
          e.dataTransfer.getData("text/warcraft-hero-id") || e.dataTransfer.getData("text/plain"),
        quest = offers[index % Math.max(1, offers.length)];
      if (heroId && quest) {
        ContentAssignments.assign("quest", index, heroId, quest.id);
        renderAutomation();
        renderJournal();
      }
    });
  ...
  roster.className = "assignment-board__roster";
  Roster.getState().heroes.forEach(hero => {
    const card = document.createElement("div"),
      available =
        hero.availability === "available" && !ContentAssignments.assignmentForHero(hero.id);
    card.className = "assignment-hero" + (available ? "" : " is-unavailable");
    card.draggable = available;
    card.innerHTML = "<strong>" + hero.name + "</strong><small>" + hero.classLabel + "</small>";
    if (available)
      card.addEventListener("dragstart", e => {
        e.dataTransfer.setData("text/warcraft-hero-id", hero.id);
      });
    roster.appendChild(card);
  });
  board.prepend(roster);
}
