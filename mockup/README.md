...
Hero advancement uses a faction-persistent **0–20 level-progress XP** field. Successful Quest / Incursion / Dungeon completion awards **1 / 2 / 3 XP** per participating hero; Raid and Siege award **0 XP**. XP clamps at 20 and never auto-levels. `WarcraftRoster.completeHeroLevelTraining(heroId)` is the Class-Hall-ready level transition: it requires 20/20, never exceeds level 5 or the active faction Keep/Base Level, and resets progress to 0 only after a successful level increase. Roster identity renders this state as a 20-segment bar, and Battle/Quest resolution stores XP reward events in authoritative result state.
