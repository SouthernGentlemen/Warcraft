...
function currentRaceMeta() {
  return currentFaction().races.find(function (race) {
    return race.id === state.race;
  });
}

function bodyMeta() {
  return state.index.body_types.find(function (body) {
    return body.id === state.body;
  });
}
...
function renderFactionControls() {
  ...
  Object.entries(state.index.factions).forEach(function (entry) {
    ...
    button.innerHTML =
      '<span class="race-faction-crest wow-faction-crest wow-faction-crest--' +
      factionId +
      '">' +
      '<img src="' +
      Icons.resolve("faction", factionId) +
      '" alt="">' +
      "</span>" +
      '<span class="race-faction-copy"><strong>' +
      escapeHtml(faction.label) +
      "</strong><small>" +
      escapeHtml(faction.exclusive_class) +
      "</small></span>";
    ...
    Tooltips.attach(
      button,
      function () {
        return factionTooltipModel(factionId, faction);
      },
      { anchor: "target" }
    );
...
function renderBodyControls() {
  ...
  state.index.body_types.forEach(function (body) {
    ...
    button.innerHTML =
      "<strong>" +
      escapeHtml(body.label) +
      "</strong>" +
      "<small>" +
      escapeHtml(body.presentation) +
      "</small>";
    Tooltips.attach(
      button,
      function () {
        return bodyTooltipModel(body);
      },
      { anchor: "target" }
    );
    button.addEventListener("click", function () {
...
function renderRaceList() {
  ...
  faction.races.forEach(function (race) {
    ...
    button.setAttribute("aria-pressed", selected ? "true" : "false");
    button.innerHTML =
      '<span class="race-option-icon wow-icon-frame wow-icon-frame--lg' +
      (selected ? " is-selected" : "") +
      '">' +
      '<img src="' +
      Icons.resolve("race", race.id, { body: state.body }) +
      '" alt="">' +
      "</span>" +
      '<span class="race-option-copy"><strong>' +
    ...
    Tooltips.attach(
      button,
      function () {
        return raceTooltipModel(race, faction);
      },
      { anchor: "target" }
    );
    button.addEventListener("click", function () {
...
function renderClasses() {
  ...
  const availableClasses =
    race && Array.isArray(race.available_classes) ? race.available_classes : [];
  ...
  state.classIndex.classes.forEach(function (classMeta) {
    ...
    button.type = "button";
    button.className =
      "race-class-button wow-class--" + classMeta.id + (isAvailable ? "" : " is-locked");
    button.setAttribute("aria-disabled", isAvailable ? "false" : "true");
    button.setAttribute(
      "aria-label",
      classMeta.label + (isAvailable ? ", available" : ", unavailable")
    );
    button.innerHTML =
      '<span class="race-class-icon wow-icon-frame wow-icon-frame--md wow-icon-frame--class-' +
      classMeta.id +
      (isAvailable ? "" : " is-locked") +
      '">' +
      '<img src="' +
      Icons.resolve("class", classMeta.id) +
      '" alt="">' +
      "</span>" +
      "<span>" +
      escapeHtml(classMeta.label) +
      "</span>";
  ...
  $("classSummary").textContent =
    ...
    " available · " +
    (state.classIndex.classes.length - availableClasses.length) +
    " locked";
}
...
async function init() {
  try {
    ...
    const loaded = await Promise.all([loadJson(RACE_INDEX), loadJson(CLASS_ROOT + "index.json")]);
    ...
    const savedFaction =
      Roster && ["alliance", "horde"].includes(Roster.getFaction())
        ? Roster.getFaction()
        : "alliance";
