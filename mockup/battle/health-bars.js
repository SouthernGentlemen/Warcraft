...
function actorHealth(actor){
  const max=Math.max(0,finite(actor&&actor.definition&&actor.definition.derived&&actor.definition.derived.maxHealth));
...
function actorWidth(actor){return widthPercent(actorHealth(actor).percent);}
...
function teamWidth(totals){return widthPercent(teamPercent(totals));}
global.BattleHealthBars=Object.freeze({actorHealth,widthPercent,actorWidth,teamTotals,teamPercent,teamWidth});
})(window);
