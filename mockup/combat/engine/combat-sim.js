...
export class CombatSimulation {
  ...
  step() {
    ...
    for (let index = 0; index < this.defs.length; index += 1) {
      ...
      if (def.ultimate && actor.ultimate >= ULTIMATE_MAX && this.hasUsefulTarget(index, def.ultimate)) {
        queue.push({ actorIndex: index, source: "ultimate", slot: -1, action: def.ultimate });
      }

      const cooldownSlot = this.firstReadyCooldown(index);
      if (cooldownSlot >= 0) {
        queue.push({ actorIndex: index, source: "cooldown", slot: cooldownSlot, action: def.cooldowns[cooldownSlot] });
      }
  ...
  tickBuffs(report) {
    for (let i = 0; i < this.state.actors.length; i += 1) {
      ...
      for (const buff of actor.buffs) {
        ...
        else report.events.push({
          type: "buff_expire", actor: i, target: i, key: buff.key,
          source: buff.source, frame: this.state.frame
        });
      }
      actor.buffs = kept;
    }
  ...
  regenerateResources(report) {
    for (let i = 0; i < this.defs.length; i += 1) {
      ...
      if (actual > 0) report.events.push({
        type: "resource_regen", actor: i, target: i, resource: def.resourceType,
        amount: actual, value: actor.resource, frame: this.state.frame
      });
    }
  }
    ...
    for (let i = 0; i < this.defs.length; i += 1) {
      ...
      for (let slot = 0; slot < actor.cooldowns.length; slot += 1) {
        ...
        if (before > 0 && cooldown.remaining === 0) {
          report.events.push({
            type: "cooldown_ready", actor: i, action: this.defs[i].cooldowns[slot].name,
            frame: this.state.frame
          });
        }
      }
  ...
  hasUsefulTarget(index, action) {
    ...
    if (action.kind === "heal") {
      return targets.some(target => this.state.actors[target].hp < this.defs[target].derived.maxHealth);
    }
    return true;
  }
  ...
  spendForAction(index, source, slot, action, report) {
    ...
    if (source === "ultimate") {
      ...
      report.events.push({
        type: "ultimate_spend", actor: index, action: action.name,
        amount: ULTIMATE_MAX, value: actor.ultimate, frame: this.state.frame
      });
      return true;
    }
    ...
    if (source === "cooldown") {
      ...
      if (action.resource !== "none") {
        ...
        actor.resource -= cost;
        report.events.push({
          type: "resource_spend", actor: index, action: action.name, resource: def.resourceType,
          amount: cost, value: actor.resource, frame: this.state.frame
        });
      }
      actor.cooldowns[slot].remaining = action.cooldown_ticks;
      actor.cooldowns[slot].carry = 0;
      report.events.push({
        type: "cooldown_start", actor: index, action: action.name,
        ticks: action.cooldown_ticks, frame: this.state.frame
      });
      return true;
    }
  ...
  outputAmount(index, action, source) {
    let amount = Math.trunc(action.power) + mulBp(this.powerStat(index, action.school), action.coefficient_bp);
  ...
  resolveAction(queued, report) {
    ...
    if (!targets.length || (action.kind === "heal" && !this.hasUsefulTarget(actorIndex, action))) return;
  ...
  resolveAgainstTarget(actorIndex, targetIndex, action, source, report) {
    if (action.canMiss) {
      ...
      if (!hit) {
        report.events.push({
          type: "miss", actor: actorIndex, target: targetIndex,
          action: action.name, frame: this.state.frame
        });
        return false;
      }
      report.events.push({
        type: "hit", actor: actorIndex, target: targetIndex,
        action: action.name, frame: this.state.frame
      });
    }
    ...
    if (action.canCrit) {
      const critChance = clampInt(
        this.defs[actorIndex].derived.critBp + (source === "auto" ? this.defs[actorIndex].talentHooks.autoCritBp : 0),
        0,
        10_000
      );
      ...
      if (critical) {
        ...
        report.events.push({
          type: "critical", actor: actorIndex, target: targetIndex,
          action: action.name, frame: this.state.frame
        });
      }
    }
  ...
  applyDamage(actorIndex, targetIndex, action, rawAmount, critical, report) {
    ...
    report.events.push({
      type: "damage", actor: actorIndex, target: targetIndex, action: action.name,
      amount: hpDamage, raw: rawAmount, absorbed, critical,
      hp: target.hp, frame: this.state.frame
    });

    if (hpDamage > 0) {
      this.addUltimate(targetIndex, 300, "damage-taken", report);
      if (this.defs[targetIndex].resourceType === "rage") this.addResource(targetIndex, 5, "damage-rage", report);
    }

    if (target.hp <= 0 && target.alive) {
      target.alive = 0;
      report.events.push({
        type: "death", actor: targetIndex, target: targetIndex,
        sourceActor: actorIndex, action: action.name, frame: this.state.frame
      });
    }
  }
  ...
  applyHeal(actorIndex, targetIndex, action, rawAmount, critical, report) {
    ...
    report.events.push({
      type: "heal", actor: actorIndex, target: targetIndex, action: action.name,
      amount: actual, raw: rawAmount, overheal, critical,
      hp: target.hp, frame: this.state.frame
    });
  }
  ...
  applyShield(actorIndex, targetIndex, action, amount, report) {
    ...
    report.events.push({
      type: "shield", actor: actorIndex, target: targetIndex, action: action.name,
      amount, value: target.shield, frame: this.state.frame
    });
  }
  ...
  applyBuff(actorIndex, targetIndex, action, report) {
    ...
    if (!effect) {
      report.events.push({
        type: "buff_unimplemented", actor: actorIndex, target: targetIndex,
        action: action.name, effect: action.effect, frame: this.state.frame
      });
      return true;
    }

    const target = this.state.actors[targetIndex];
    target.buffs = target.buffs.filter(buff => !(buff.key === effect.key && buff.source === actorIndex));
    target.buffs.push({
      key: effect.key,
    ...
    report.events.push({
      type: "buff_apply", actor: actorIndex, target: targetIndex, action: action.name,
      key: effect.key, valueBp: effect.valueBp, ticks: effect.duration,
      frame: this.state.frame
    });
    return true;
  }
  ...
  addUltimate(index, amount, reason, report) {
    ...
    if (actual <= 0) return;
    report.events.push({
      type: "ultimate_gain", actor: index, target: index, reason,
      amount: actual, value: actor.ultimate, frame: this.state.frame
    });
  ...
  addResource(index, amount, reason, report) {
    ...
    if (actual <= 0) return;
    report.events.push({
      type: "resource_gain", actor: index, target: index, resource: def.resourceType,
      reason, amount: actual, value: actor.resource, frame: this.state.frame
    });
  }
  ...
  checkRoundEnd(report) {
    ...
    report.events.push({
      type: "round_end", winnerTeam: this.state.winnerTeam, frame: this.state.frame
    });
  }
