...
function model(faction){
  ...
  return{
    ...
    label:"Day "+Math.max(1,Number(clock.day)||1)+" · "+(phase==="night"?"Night":"Day")
  };
}
function render(root,faction){
  ...
  root.innerHTML='<span class="campaign-clock__orb" aria-hidden="true"></span><span class="campaign-clock__copy"><small>Campaign Time</small><strong>'+value.label+'</strong></span>';
  return value;
}
function hydrate(root){
  const scope=root&&typeof root.querySelectorAll==="function"?root:global.document;
  if(!scope)return[];
  return Array.from(scope.querySelectorAll("[data-campaign-clock]")).map(node=>render(node,node.dataset.campaignFaction||undefined));
}
if(typeof global.addEventListener==="function")global.addEventListener("warcraft:campaign-changed",event=>{
  const reason=event&&event.detail&&event.detail.reason;
  if(["clock","faction","reset"].includes(reason))hydrate(global.document);
});
global.WarcraftCampaignClock=Object.freeze({model,render,hydrate});
})(window);
