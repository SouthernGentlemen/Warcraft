...
function defaultQuests(){return [1,2,3,4,5].map((tier,i)=>({id:"quest-tier-"+tier,tier,requiredHeroes:[1,3,5,10,20][i],status:"available",heroIds:[],completedCount:0,reward:{gold:tier*250,meta:"quest_marks",amount:tier}}));}
function defaults(){return {faction:"alliance",heroes:DEFAULT_HEROES.map(h=>Object.assign({},h,{equipment:blankEquipment(),talentBuild:{primarySpec:h.spec,picks:[]}})),loadouts:Array.from({length:5},(_,i)=>({id:"party-"+(i+1),name:"Party "+(i+1),size:3,heroIds:[],ready:false})),quests:defaultQuests()};}
function normalize(raw){const base=defaults(),saved=raw&&typeof raw==="object"?raw:{};base.faction=["alliance","horde"].includes(saved.faction)?saved.faction:"alliance";const byId=new Map((saved.heroes||[]).map(h=>[h.id,h]));base.heroes=base.heroes.map(h=>{const s=byId.get(h.id)||{};return Object.assign({},h,s,{equipment:Object.assign(blankEquipment(),s.equipment||{}),talentBuild:Object.assign({},h.talentBuild,s.talentBuild||{})});});const ls=Array.isArray(saved.loadouts)?saved.loadouts:[];base.loadouts=base.loadouts.map((l,i)=>{const s=ls[i]||{};const size=PARTY_SIZES.includes(Number(s.size))?Number(s.size):3;return {id:l.id,name:String(s.name||l.name).slice(0,40),size,heroIds:[...new Set((s.heroIds||[]).filter(id=>base.heroes.some(h=>h.id===id)))].slice(0,size),ready:Boolean(s.ready)};});const qs=Array.isArray(saved.quests)?saved.quests:[];base.quests=base.quests.map((q,i)=>Object.assign({},q,qs[i]||{},{tier:q.tier,requiredHeroes:q.requiredHeroes,heroIds:[...new Set(((qs[i]||{}).heroIds||[]).filter(id=>base.heroes.some(h=>h.id===id)))]}));return base;}
...
function setFaction(faction){const next=String(faction||"").toLowerCase();if(!["alliance","horde"].includes(next))throw new Error("Faction must be Alliance or Horde.");if(state.faction===next)return state.faction;state.faction=next;save();return state.faction;}
...
function dispatchQuest(tier,heroIds){const quest=state.quests.find(q=>q.tier===tier);if(!quest)throw new Error("Unknown quest tier.");const ids=[...new Set(heroIds||[])];if(ids.length!==quest.requiredHeroes)throw new Error("Tier "+tier+" requires exactly "+quest.requiredHeroes+" hero"+(quest.requiredHeroes===1?"":"es")+".");const selected=ids.map(hero);if(selected.some(h=>!h||h.availability!=="available"))throw new Error("Every dispatched hero must be available.");quest.heroIds=ids;quest.status="active";selected.forEach(h=>h.availability="on-quest");save();return quest;}
function completeQuest(tier){const quest=state.quests.find(q=>q.tier===tier);if(!quest||quest.status!=="active")return null;quest.heroIds.map(hero).filter(Boolean).forEach(h=>h.availability="available");quest.status="completed";quest.completedCount=(quest.completedCount||0)+1;quest.heroIds=[];save();return quest;}
function reset(){state=defaults();save();}
global.WarcraftRoster=Object.freeze({PARTY_SIZES,getState,getFaction,setFaction,hero,validateRaceClass,updateHero,setEquipment,setTalentBuild,validateLoadout,updateLoadout,dispatchQuest,completeQuest,reset});
})(window);
