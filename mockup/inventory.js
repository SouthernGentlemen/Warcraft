...
function itemTooltipModel(item) {
  ...
  const primary = (item.stats || []).filter(line =>
    ["Strength", "Agility", "Intellect", "Stamina"].includes(line.stat)
  );
  const secondary = (item.stats || []).filter(
    line => !["Strength", "Agility", "Intellect", "Stamina"].includes(line.stat)
  );
  const stats = primary
    .concat(secondary)
    .map(line => ({ label: line.stat, value: "+" + line.value }));
...
function filteredItems() {
  ...
  return state.items
    .filter(item => {
      ...
      if (
        query &&
        ![item.name, item.family, item.slot, item.quality].join(" ").toLowerCase().includes(query)
      )
        return false;
      return true;
    })
...
function renderItem(item) {
  ...
  button.type = "button";
  button.className =
    "inventory-item " + qualityClass(item) + (holders.length ? " is-equipped" : "");
  ...
  button.innerHTML =
    '<span class="inventory-item__icon wow-icon-frame wow-icon-frame--md ' +
    qualityFrameClass(item) +
    '">' +
    '<img src="' +
    Icons.iconUrl(item.icon) +
    '" alt="">' +
    '<span class="inventory-item__tier">T' +
    item.tier +
    "</span>" +
    "</span>" +
    ...
    (holders.length
      ? "<strong>Equipped</strong><small>" +
        escapeHtml(holders.map(hero => hero.name).join(", ")) +
        "</small>"
      : "<strong>Stored</strong><small>Unequipped</small>") +
    "</span>";
...
function bindControls() {
  [
    ...
    node.addEventListener(
      node.tagName === "INPUT" && node.type === "search" ? "input" : "change",
      renderInventory
    );
  });
}
